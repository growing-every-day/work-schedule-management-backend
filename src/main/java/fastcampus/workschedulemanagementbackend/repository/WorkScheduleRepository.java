...
public interface WorkScheduleRepository extends JpaRepository<WorkSchedule, Long> {
    @Query(value = "select DISTINCT c from WorkSchedule c left join fetch c.userAccount where (function('date_format', s.start, '%Y, %m') = function('date_format', :date, '%Y, %m') or function('date_format', s.end, '%Y, %m') = function('date_format', :date, '%Y, %m'))")
